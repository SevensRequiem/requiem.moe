...
if(!empty($_POST['content']) && !empty($_POST['title']) && (has_admin($guildid, $adminid))){
...
// Get all post UUIDs and sort by creation date of post.md file (most recent first)
...
foreach ($postDirs as $postDir) {
    ...
    $postUrl = './data/posts/' . $postId . '/';
    // display post data
    echo '<fieldset class="blogpost">';
    echo '<legend>' . $postTitle . '.txt</legend>';
    echo $Parsedown->text($postContent); // Parse post content with Parsedown
    if (!empty($postImage)) {
...
$comments = simplexml_load_file($comments_file);

// Display existing comments
if ($comments->comment) {
    foreach ($comments->comment as $comment) {
        echo '<div class="comment">';
        echo '<p><strong>' . $comment->author . '</strong> said on ' . $comment->date . ':</p>';
...
if (isset($_SESSION['user'])) {
    ...
    echo '<input type="text" name="username" value="' . $_SESSION['user']['username'] . '" disabled>';
